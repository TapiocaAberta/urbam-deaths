$(function() {
	...
	var values = [ [ "JANEIRO", 0, 0 ], [ "FEVEREIRO", 0, 0 ],
			[ "MARCO", 0, 0 ], [ "ABRIL", 0, 0 ], [ "MAIO", 0, 0 ],
			[ "JUNHO", 0, 0 ], [ "JULHO", 0, 0 ], [ "AGOSTO", 0, 0 ],
			[ "SETEMBRO", 0, 0 ], [ "OUTUBRO", 0, 0 ], [ "NOVEMBRO", 0, 0 ],
			[ "DEZEMBRO", 0, 0 ] ];

	var daysData = [];

	function compareLastMonthDeaths() {
	...
	function getCurrentDeaths() {
		$.getJSON('/api/deaths/direto/days/current-mount/count',
				function(data) {
					daysData = data;
				});
	}
	...
	function drawChart2() {
		...
		var options = {
			chart : {
				title : 'Óbitos por dia do mês atual registrados na Urbam',
				subtitle : 'Com funeral Direto (sem velório) excluindo natimortos e recém nacidos'
			},
			width : 900,
			height : 500,
